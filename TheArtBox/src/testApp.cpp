...
void testApp::setup(){
	...
	//DImager
	cout << "Camera W*H is " << dimager.getWidth() << "*" << dimager.getHeight() << endl;
	tex.allocate( dimager.getWidth(), dimager.getHeight(), GL_RGBA );
	img.allocate( dimager.getWidth(), dimager.getHeight(), OF_IMAGE_GRAYSCALE );
...
void testApp::draw(){
	...
	// Pixels
	if(tex.bAllocated()) {
		tex.draw(dimager.getWidth(), 0, dimager.getWidth(), dimager.getHeight());
	}

	//Image
	img.draw(dimager.getWidth()*2, 0, dimager.getWidth(), dimager.getHeight());
